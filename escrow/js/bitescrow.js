$(function() {
	...
	$('#generate-all').click(function(e) {
		...
		if(escrow != null) {
			var invA = escrow.invitationA;
			var invB = escrow.invitationB;
			var code1 = invA;
			var payment = Bitcoin.Escrow.CreatePaymentCode(code1);
			if(payment != null) {
				var address = payment.address;
				var einvp = payment.invitationP;
				var cfrmp = payment.confirmation;
			}
			$('#sellerInvite').val(invA);
			$('#buyerInvite').val(invB);
			$('#address').val(address);
			$('#paymentInvite').val(einvp);
		}
